...
xi.amanTrove.greysonReturnRoll = function(player, tradedOrbType)
    ...
    elseif roll <= rechargeChance + 10 then -- The next 10%
        return xi.item.SPECIAL_GOBBIEDIAL_KEY
...
xi.amanTrove.onGreysonTrade = function(player, npc, trade)
    local enabled               = xi.settings.main.ENABLE_AMAN_TROVE     == 1
    local playerAgeDays         = (os.time() - player:getTimeCreated()) / 86400
    local playerOldEnough       = playerAgeDays >= xi.settings.main.AMAN_TROVE_MINIMUM_AGE_DAYS
    local playerLevelHighEnough = player:getMainLvl() >= xi.settings.main.AMAN_TROVE_MINIMUM_LEVEL
