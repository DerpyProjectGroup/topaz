...
-- TODO: Split stairs away from basement loop
...
-- Direction 0 = West -> East; 1 = East -> West
...
local setZipPath = function(mob, door, label)
    if door then
        if door:getAnimation() == xi.anim.OPEN_DOOR then
            if currentDirection == 0 then -- West to East
                mob:pathThrough(paths[label + 1], xi.path.flag.COORDS)
                if label + 1 == 5 then
                    currentDirection = 1 - currentDirection
                end
            else -- East to West
                mob:pathThrough(paths[label], bit.bor(xi.path.flag.COORDS, xi.path.flag.REVERSE))
                if label == 1 then
                    currentDirection = 1 - currentDirection
                end
            end
...
entity.onMobSpawn = function(mob)
    -- Check distance on each of the spawn points in nm_spawn_points sql to find start point
    if mob:checkDistance(spawnPoints.blueShort) <= 10 then
        currentDirection = 0 -- West to East
        mob:pathThrough(paths['blueShortSideSpawnPath'], xi.path.flag.COORDS)

    elseif mob:checkDistance(spawnPoints.blueLong) <= 10 then
        currentDirection = 0 -- West to East
        mob:pathThrough(paths['blueBasementSpawnPath'], xi.path.flag.COORDS)

    elseif mob:checkDistance(spawnPoints.yellowShort) <= 10 then
        currentDirection = 1 -- East to West
        mob:pathThrough(paths['yellowShortSideSpawnPath'], xi.path.flag.COORDS)

    elseif mob:checkDistance(spawnPoints.yellowLong) <= 10 then
        currentDirection = 1 -- East to West
        mob:pathThrough(paths['yellowBasementSpawnPath'], xi.path.flag.COORDS)
    end
end
